...
export const getStudentDashData = async (userId) => {
  const [ mylistings] = await db.query ("SELECT * FROM products WHERE seller_id = ? ", [userId]);

  const [myOrders] = await db.query ("SELECT * FROM orders WHERE user_id =?", [userId]);
...
export const getAdminStats = async () => {
  ...
  const [[reports]] = await db.query ("SELECT COUNT(*) AS pendingReports FROM reports WHERE status = 'pending'");
